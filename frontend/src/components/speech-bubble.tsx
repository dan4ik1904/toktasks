...
export function SpeechBubble({
  ...
  return (
    <motion.section
      ...
      aria-live="polite"
      className="rounded-2xl border border-[var(--gold)]/40 bg-[var(--surface)] p-4"
    >
      <p className="pb-1 text-[11px] tracking-widest text-[var(--gold)] uppercase">
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          ...
          <p className="pt-1 text-sm text-[var(--muted)]">{speech.ru}</p>
        </div>
        <button
          onClick={() => void ttsSpeak(speech.tt, voice)}
          aria-label="Прослушать реплику"
          className={cn(
            "flex size-10 shrink-0 items-center justify-center rounded-full bg-[var(--gold-soft)] text-[var(--gold)] hover:brightness-95",
